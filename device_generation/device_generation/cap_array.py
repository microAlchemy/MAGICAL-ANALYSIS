...
class custom_cap:
    def __init__(self, name, w, sp, nf, l, m_bot=3, m_top=5, attr=[], f_tip=0.14, flatten=True):
        ...
        self.m_bot = m_bot
        self.m_top = m_top
        # Clamp to SKY130's 5-metal stack to avoid M6/VIA5 usage.
        if self.m_top > 5:
            self.m_top = 5
        if self.m_bot > self.m_top:
            self.m_bot = self.m_top
        self._route_top = self.m_top if self.m_top >= 1 else 1
        self.plus = Pin('PLUS')
...
# MOMCAP Finger CORE
    ...
    def lvs_layer(self):
        ...
        sp['CO']['CO'] = temp
        self.bulk.add_shape('M' + str(self._route_top), sub_cell1.get_bounding_box())
        self.bulk.add_shape('M' + str(self._route_top), sub_cell2.get_bounding_box())
        # PO Dummy Layer
...
class unit_cap(custom_cap):
    ...
    def top_ext(self):
        sub_cell = basic.metal_hori(self.con2[1][0]-self.con2[0][0], self.con2[1][1]-self.con2[0][1], lay=self._route_top)
        sub_cell2 = gdspy.CellReference(sub_cell, (self.con2[0][0], self.con2[0][1]))
        self.cell.add(sub_cell2)
        ext_dist = 2 * self.od25_p2[0] - self.od25_p1[0]
        hori_ext = gdspy.Rectangle((self.con2[0][0], self.con2[0][1]), (ext_dist, self.con2[1][1]), layer['M'+str(self._route_top)])
...
class cap_array:
    ...
    def connect_top_2(self, width=0.2, con_l=0.6):
        ...
        for i in range(1, self.col_bit+1):
            ...
            self.cell.add(col_shape)
        for y in self.top_y:
            con_shape = gdspy.Rectangle([self.bound[1][0], y], [self.col_p2[0]+2*self.col_bit*width+2*width, y+self.array_cell.con2[1][1]-self.array_cell.con2[0][1]], layer['M'+str(self.array_cell._route_top)])
            self.cell.add(con_shape)
        # Connect according to interdigitation
        col_coord = self.interdigitation_code(self.col_bit+1)
        vert_cell = basic.metal_vert(width, con_l, self.array_cell._route_top)
        bit = 0
